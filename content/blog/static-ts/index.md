...
STS 和 StrongScript 也有一些区别，首先，StrongScript 的类型系统保证在不存在类型向下转换的情况下，具体类`C`的变量引用名义上为`C`的子类型的对象或为`null`。STS 使用 TypeScript 的类型推断和类型检查，没有做这方面的修改。在 STS 中，类`C`的变量会被动态检查（通过查找字段/方法），确定其值是否为类`C`的名义子类型。其次，STS 会删除强制类型转换，StrongScript 会在运行时对其进行检查，而如上所述，STS 会在取消引用时（成员/字段查找）执行检察。Strong 允许使用额外的属性拓展类的对象，而 STS 目前不允许进行这样的拓展。
